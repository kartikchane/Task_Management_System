...
import {generateDailyWork} from './scheduler.js';
...
r.post('/daily-templates',allow('superadmin','admin'),validateBody(schemas.dailyTemplate),asyncHandler(async(req,res)=>{if(req.user.role==='admin'){if(!req.user.department){res.status(400);throw new Error('Create your department before adding daily templates')}const teamIds=await teamEmployeeIds(req,User);if(req.body.employees?.some(id=>!teamIds.some(t=>String(t)===String(id)))){res.status(403);throw new Error('Select employees only from your team')}req.body.department=dep(req.user.department)}const x=await DailyTaskTemplate.create({...req.body,createdBy:req.user._id});await logActivity(req,'create','DailyTaskTemplate',x._id);try{await generateDailyWork(io)}catch(err){console.error('Immediate daily-work generation failed',err)}emitOps(io,x.department,'daily-template:updated',{id:x._id,action:'created'});res.status(201).json(x)}));
r.patch('/daily-templates/:id',allow('superadmin','admin'),validateBody(schemas.dailyTemplate.partial()),asyncHandler(async(req,res)=>{const q={_id:req.params.id,...(req.user.role==='superadmin'?{}:{department:dep(req.user.department)})};if(req.user.role==='admin'){const teamIds=await teamEmployeeIds(req,User);if(req.body.employees?.some(id=>!teamIds.some(t=>String(t)===String(id)))){res.status(403);throw new Error('Select employees only from your team')}req.body.department=dep(req.user.department)}const x=await DailyTaskTemplate.findOneAndUpdate(q,req.body,{new:true,runValidators:true});try{await generateDailyWork(io)}catch(err){console.error('Immediate daily-work generation failed',err)}emitOps(io,x?.department,'daily-template:updated',{id:req.params.id,action:'updated'});res.json(x)}));
...
r.post('/daily-templates/generate',allow('superadmin','admin'),asyncHandler(async(req,res)=>{const target=req.body.date||date(),calendarDay=new Date(target+'T00:00:00'),day=calendarDay.getDay(),monthDay=calendarDay.getDate();if(await Holiday.exists({date:target}))return res.json({message:'Holiday - no work generated',created:0});const templates=await DailyTaskTemplate.find({active:true,$or:[{cadence:'daily',workingDays:day},{cadence:'weekly',workingDays:day},{cadence:'monthly',monthlyDay:monthDay},{cadence:{$exists:false},workingDays:day}],...(req.user.role==='superadmin'?{}:{department:dep(req.user.department)})});let created=0;const departments=new Set();for(const t of templates){const userScope=req.user.role==='admin'?{manager:req.user._id}:{},users=t.assigneeMode==='selected'?await User.find({...userScope,_id:{$in:t.employees},status:'active'}):await User.find({...userScope,department:t.department,role:'employee',status:'active'});let taskTitles;if(t.rotation&&t.checklist?.length){const sortedDays=[...new Set(t.workingDays?.length?t.workingDays:[1,2,3,4,5,6])].sort((a,b)=>a-b);const pos=sortedDays.indexOf(day);taskTitles=pos===-1?[]:[t.checklist[pos%t.checklist.length]]}else{taskTitles=t.checklist?.length?t.checklist:[t.title]}if(!taskTitles.length)continue;for(const u of users){if(await Leave.exists({employee:u._id,status:'approved',fromDate:{$lte:target},toDate:{$gte:target}}))continue;const existing=await DailyWork.findOne({employee:u._id,date:target}).select('assignedTasks');const already=new Set((existing?.assignedTasks||[]).filter(x=>String(x.template)===String(t._id)).map(x=>x.title));const toAdd=taskTitles.filter(title=>!already.has(title));if(!toAdd.length)continue;const newTasks=toAdd.map(title=>({title,description:t.description,dueTime:`${String(t.dueHour??18).padStart(2,'0')}:00`,priority:t.priority||'medium',assignedBy:t.createdBy,template:t._id}));const row=await DailyWork.findOneAndUpdate({employee:u._id,date:target},{$setOnInsert:{department:u.department},$push:{assignedTasks:{$each:newTasks}}},{upsert:true,new:true});const newlyAdded=row.assignedTasks.slice(-newTasks.length);for(const task of newlyAdded){await notify(io,u._id,{type:'daily-assigned',title:'New daily task assigned',message:task.title,link:'/daily-work',meta:{kind:'daily',workId:row._id,taskId:task._id}})}departments.add(String(u.department));io.to(String(u._id)).emit('daily-work:updated',row);created+=newlyAdded.length}}for(const department of departments){emitOps(io,department,'daily-work:generated',{date:target});emitOps(io,department,'dashboard:updated')}res.json({message:'Recurring work generation complete',created})}));
r.get('/attendance/today',allow('employee','admin','superadmin'),asyncHandler(async(req,res)=>res.json(await Attendance.findOne({employee:req.user._id,date:date()}))));
r.post('/attendance/check-in',allow('employee','admin','superadmin'),asyncHandler(async(req,res)=>{const now=new Date();const x=await Attendance.findOneAndUpdate({employee:req.user._id,date:date()},{$setOnInsert:{department:dep(req.user.department),checkIn:now,status:'present'}},{new:true,upsert:true});emitOps(io,dep(req.user.department),'attendance:updated',x);emitOps(io,dep(req.user.department),'dashboard:updated');res.json(x)}));
r.post('/attendance/check-out',allow('employee','admin','superadmin'),asyncHandler(async(req,res)=>{const x=await Attendance.findOne({employee:req.user._id,date:date()});if(!x?.checkIn){res.status(400);throw new Error('Check in first')}x.checkOut=new Date();x.workedMinutes=Math.max(0,Math.round((x.checkOut-x.checkIn)/60000)-x.breakMinutes);await x.save();emitOps(io,x.department,'attendance:updated',x);emitOps(io,x.department,'dashboard:updated');res.json(x)}));
...
r.get('/leaves',asyncHandler(async(req,res)=>{const teamScope=await teamEmployeeScope(req);const q=req.user.role==='employee'?{employee:req.user._id}:req.user.role==='superadmin'?{}:{$or:[{employee:req.user._id},teamScope]};res.json(await Leave.find(q).populate('employee','name email').populate('reviewedBy','name').sort('-createdAt'))}));
r.post('/leaves',allow('employee','admin','superadmin'),validateBody(schemas.leave),asyncHandler(async(req,res)=>{const x=await Leave.create({...req.body,employee:req.user._id,department:dep(req.user.department)});if(req.user.manager){await notify(io,req.user.manager,{type:'leave-request',title:'Leave request pending',message:`${req.user.name} requested leave from ${x.fromDate} to ${x.toDate}.`,link:'/approvals'})}else if(req.user.role!=='superadmin'){const superadmins=await User.find({role:'superadmin',status:'active'}).select('_id');for(const sa of superadmins)await notify(io,sa._id,{type:'leave-request',title:'Leave request pending',message:`${req.user.name} requested leave from ${x.fromDate} to ${x.toDate}.`,link:'/approvals'})}emitOps(io,x.department,'leave:updated',x);emitOps(io,x.department,'calendar:updated');res.status(201).json(x)}));
...
r.get('/calendar',asyncHandler(async(req,res)=>{const from=req.query.from||date(),to=req.query.to||date();const taskQ=req.user.role==='employee'?{assignedTo:req.user._id}:await teamTaskScope(req);const leaveQ=req.user.role==='employee'?{employee:req.user._id}:{...(await teamEmployeeScope(req)),fromDate:{$lte:to},toDate:{$gte:from}};const dailyWorkQ=req.user.role==='employee'?{employee:req.user._id}:await teamEmployeeScope(req);const [tasks,leaves,holidays,dailyRows]=await Promise.all([Task.find({...taskQ,dueDate:{$gte:new Date(from),$lte:new Date(to+'T23:59:59')}}).select('title dueDate status priority assignedTo').populate('assignedTo','name'),Leave.find(leaveQ).populate('employee','name'),Holiday.find({date:{$gte:from,$lte:to}}),DailyWork.find({...dailyWorkQ,date:{$gte:from,$lte:to}}).select('employee date assignedTasks').populate('employee','name').lean()]);const dailyTasks=[];for(const row of dailyRows){for(const t of row.assignedTasks||[])dailyTasks.push({_id:t._id,title:t.title,date:row.date,dueTime:t.dueTime,status:t.status,priority:t.priority,employee:row.employee})}res.json({tasks,leaves,holidays,dailyTasks})}));
