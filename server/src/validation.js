...
export const schemas={
  ...
  task:z.object({title:trimmed,description:trimmed,project:objectId.optional().or(z.literal('')),department:objectId,assignedTo:objectId,priority:z.enum(['low','medium','high','critical']).optional(),dueDate:z.string().min(1),status:z.enum(['completed','in-progress','pending','overdue','not-applicable','submitted']).optional(),completionRequirements:z.array(z.enum(['photo','document','excel','invoice','screenshot','customer-confirmation','remark'])).optional()}),
  ...
  dailyTemplate:z.object({title:trimmed,description:z.string().trim().optional(),department:objectId,cadence:z.enum(['daily','weekly','monthly']).optional(),priority:z.enum(['low','medium','high','critical']).optional(),assigneeMode:z.enum(['department','selected']).optional(),employees:z.array(objectId).optional(),checklist:z.array(z.string().trim()).optional(),rotation:z.boolean().optional(),workingDays:z.array(z.coerce.number().min(0).max(6)).optional(),monthlyDay:z.coerce.number().min(1).max(31).optional(),dueHour:z.coerce.number().min(0).max(23).optional(),active:z.boolean().optional()}),
