...
export const schemas={
  ...
  user:z.object({name:trimmed,email:z.string().email(),password:z.string().min(8).optional(),role:z.enum(['superadmin','admin','employee']).optional(),department:objectId.optional().or(z.literal('')),manager:objectId.optional().or(z.literal('')),designation:z.string().trim().optional(),employeeCode:z.string().trim().optional(),joiningDate:z.string().optional(),skills:z.array(z.string().trim()).optional(),phone:z.string().trim().optional(),status:z.enum(['active','blocked','resigned','archived']).optional()}),
  ...
  task:z.object({title:trimmed,description:trimmed,project:objectId,department:objectId,assignedTo:objectId,priority:z.enum(['low','medium','high','critical']).optional(),dueDate:z.string().min(1),status:z.enum(['todo','in-progress','submitted','approved','rework','rejected','cancelled']).optional()}),
  taskProgress:z.object({progress:z.coerce.number().min(0).max(100)}),
  taskComment:z.object({text:trimmed.max(2000)}),
  taskSubmit:z.object({note:trimmed.max(2000)}),
  taskReview:z.object({decision:z.enum(['approved','rework','rejected']),note:z.string().trim().optional(),points:z.coerce.number().min(0).optional(),quality:z.coerce.number().min(0).optional(),rating:z.coerce.number().min(0).max(5).optional()}),
  ...
  register:z.object({name:trimmed,email:z.union([z.string().email(),z.literal('')]).default(''),password:z.string().min(8),role:z.enum(['admin','employee']),department:objectId.optional().or(z.literal('')),designation:z.string().trim().optional(),phone:z.string().trim().default('')}).refine(x=>(x.email&&x.email.trim())||(x.phone&&x.phone.trim()),{message:'Email or phone number is required'}),
