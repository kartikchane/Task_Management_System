...
export default function Login(){
  ...
  const switchMode=next=>{
    setMode(next);
    if(next==='register')setForm({...form,name:'',email:'',password:'',role:'employee',department:'',designation:'',phone:''});
    if(next==='login')setForm({...form,phone:'',password:''});
  };
  const submit=async e=>{
    ...
    try{
      if(mode==='reset'){
        ...
        setUser(data.user);
        toast.success(`${data.user.role==='admin'?'Admin':'Employee'} account registered`);
        nav('/');
      }else{
