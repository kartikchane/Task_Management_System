...
export default function Profile(){
  ...
  return <>
    ...
    <div className="profile-layout"><aside className="card profile-summary"><div className="avatar xl">{user.name[0]}</div><h2>{user.name}</h2><p>{user.designation||user.role}</p><span>{user.email}</span>{user.role==='employee'&&<span>Employee ID: <b>{user.employeeCode||'Not assigned'}</b></span>}<button className={tab==='profile'?'active':''} onClick={()=>setTab('profile')}>Personal information</button><button className={tab==='security'?'active':''} onClick={()=>setTab('security')}>Security</button></aside><section className="card">{tab==='profile'?<form onSubmit={save} className="form-grid two"><h3 className="span-2">Personal information</h3>{user.role==='employee'&&<Field label="Employee ID"><input value={user.employeeCode||'Not assigned'} disabled/></Field>}<Field label="Full name"><input name="name" defaultValue={user.name}/></Field><Field label="Designation"><input name="designation" defaultValue={user.designation}/></Field><Field label="Phone"><input name="phone" defaultValue={user.phone}/></Field><Field label="Skills"><input name="skills" defaultValue={user.skills?.join(', ')}/></Field><div className="form-actions span-2"><Button variant="primary">Save changes</Button></div></form>:<form onSubmit={pass} className="form-grid"><h3>Change password</h3><Field label="Current password"><input type="password" name="currentPassword" required/></Field><Field label="New password"><input type="password" name="newPassword" minLength="8" required/></Field><div className="form-actions"><Button variant="primary">Update password</Button></div></form>}</section></div>
  </>;
}
