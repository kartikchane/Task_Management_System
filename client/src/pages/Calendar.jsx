...
export default function Calendar(){
  const [d,setD]=useState({tasks:[],leaves:[],holidays:[],dailyTasks:[]}),[loading,setLoading]=useState(true),[error,setError]=useState('');
  ...
  useRefresh(load);
  return <>
    <div className="page-head"><div><h1>Work Calendar</h1><p>Tasks, daily work, leave and holidays in one view.</p></div></div>
    {loading?<Skeleton/>:error?<Empty title="Unable to load calendar" text={error}/>:<div className="grid cols-3">
      ...
      <section className="card daily-form"><h3>Daily Work Tasks</h3>{d.dailyTasks?.length?d.dailyTasks.map(x=><p key={x._id}><b>{x.title}</b><br/>{x.employee?.name?x.employee.name+' · ':''}{x.date}{x.dueTime?' · Due '+x.dueTime:''}</p>):<Empty title="No daily work tasks" text="Daily Work tasks scheduled this month will appear here."/>}</section>
