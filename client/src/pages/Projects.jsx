...
import { Plus, FolderKanban, Calendar, Users, RefreshCw, Pencil, Trash2 } from "lucide-react";
...
export default function Projects() {
  ...
  const load = useCallback(() => {
    ...
    return Promise.all([
      api.get("/projects"),
      ...(user.role !== "employee" ? [api.get("/departments"), api.get("/users")] : []),
    ])
      .then((r) => {
        setRows(r[0].data);
    ...
    () =>
      rows.filter(
        (x) =>
          ...
          (!search || `${x.name} ${x.code} ${x.description || ""}`.toLowerCase().includes(search.toLowerCase())),
      ),
    [rows, search, status],
  );
  ...
  return (
    <>
      <PageHead title="Projects" subtitle="Track delivery, ownership, milestones and team allocation." />
      <section className="gp-panel">
        ...
        {loading ? (
          ...
          <Empty title="Unable to load projects" text={error} />
        ) : filtered.length ? (
          <div className="grid cols-3 project-grid">
            {filtered.map((x) => (
              <article className="card project-card" key={x._id}>
                ...
                <div className="project-meta">
                  <span>
                    <Calendar /> {x.endDate ? new Date(x.endDate).toLocaleDateString("en-IN") : "No deadline"}
                  </span>
                  <span>
                    <Users /> {x.members?.length || 0} members
                  </span>
                </div>
                ...
                {user.role !== "employee" && (
                  <div className="card-actions">
                    ...
                    <button title="Delete project" onClick={() => del(x._id)}>
                      <Trash2 />
                    </button>
                  </div>
                )}
          ...
          <Empty
            ...
            text="Projects matching your filters will appear here."
          />
        )}
      </section>
      {modal && (
        ...
        >
          <form onSubmit={save} className="form-grid two">
            ...
            <Field label="Department">
              <select name="department" defaultValue={edit?.department?._id || ""} required>
            ...
            <Field label="Manager">
              <select name="manager" defaultValue={edit?.manager?._id || ""}>
            ...
            <Field label="Priority">
              <select name="priority" defaultValue={edit?.priority || "medium"}>
            ...
            <Field label="Status">
              <select name="status" defaultValue={edit?.status || "planning"}>
                <option>planning</option>
                <option>active</option>
                <option>on-hold</option>
                <option>completed</option>
            ...
            <Field label="Start date">
              <input type="date" name="startDate" defaultValue={toDateInput(edit?.startDate)} />
            </Field>
            <Field label="End date">
              <input type="date" name="endDate" defaultValue={toDateInput(edit?.endDate)} />
            </Field>
            <Field label="Members">
              <select multiple name="members" defaultValue={edit?.members?.map((x) => x._id) || []}>
                {people
                  .filter((x) => x.role === "employee")
                  .map((x) => (
                    <option key={x._id} value={x._id}>
            ...
            <div className="form-actions span-2">
              ...
              <Button variant="primary">{edit ? "Save project" : "Create project"}</Button>
            </div>
          </form>
        </Modal>
      )}
