...
import api from "../api";
import { Button, Badge, Empty, Skeleton, PageHead, DIcon } from "../components/UI";
...
function EmployeeAttendance() {
  ...
  const statusLabel = x?.status ? x.status.replace("-", " ") : "Not Checked In";
  const time = (v) => (v ? new Date(v).toLocaleTimeString("en-IN", { hour: "2-digit", minute: "2-digit" }) : "-");
  return (
    <>
      <PageHead title="Attendance" subtitle="Track check-in, check-out and working hours." />
      {loading ? (
        <Skeleton />
      ) : (
        <section className="gp-card att-card">
          <div className="gp-card-head">
            <div>
              <h3>Today</h3>
              <p>{error || "Your attendance record for the current working day."}</p>
            </div>
            <span className={"pill " + (tones[x?.status] || "")}>{statusLabel}</span>
          </div>
          <div className="notice policy">
            ...
            <div>
              ...
              <p>
                Check in and check out record your working time. Final attendance status is reviewed and marked by your
                manager.
              </p>
            </div>
          </div>
          <div className="att-grid">
            <div className="att-item">
              <div className="stat-tile tone-navy">
                <DIcon name="clock" />
              </div>
              <div>
                <span>Check in</span>
            ...
            <div className="att-item">
              <div className="stat-tile tone-green" style={{ "--tone": "#05692a" }}>
                <svg viewBox="0 0 24 24" fill="currentColor" aria-hidden="true">
                  <rect x="2.5" y="11" width="5.2" height="11" rx="1.2" />
                  <rect x="9.4" y="4" width="5.2" height="18" rx="1.2" />
                  <rect x="16.3" y="8" width="5.2" height="14" rx="1.2" />
                </svg>
              </div>
              <div>
                <span>Worked</span>
                <b>{x?.workedMinutes || 0} min</b>
              </div>
            </div>
            <div className="att-item">
          ...
          <div className="att-actions">
            <Button variant="primary lg" onClick={() => act("check-in")} disabled={!!x?.checkIn}>
              <LogIn />
              Check in
            </Button>
            <Button variant="outline-dark lg" onClick={() => act("check-out")} disabled={!x?.checkIn || !!x?.checkOut}>
              <LogOut />
              Check out
            </Button>
          </div>
    ...
    <>
      <div className="page-head">
        <div>
          ...
          <p>Review employee check-ins, check-outs and working minutes.</p>
        </div>
      </div>
      <div className="notice success">
        <Clock3 />
        <div>
          <b>Manual attendance review</b>
          <p>
            System records check-in, check-out and worked minutes only. Managers mark late, half day, leave or absent as
            per company policy.
          </p>
        </div>
      </div>
      ...
      {loading ? (
        ...
        <Empty title="Unable to load attendance" text={error} />
      ) : rows.length ? (
                ...
                <tr key={x._id}>
                  <td>
                    <div className="person">
                      ...
                      <div>
                        <b>{x.employee?.name || "Unknown employee"}</b>
                        <span>{x.employee?.designation || x.employee?.email || "Employee"}</span>
                      </div>
                    </div>
                  </td>
                  <td>{x.date}</td>
                  <td>{x.checkIn ? new Date(x.checkIn).toLocaleTimeString("en-IN") : "-"}</td>
                  <td>{x.checkOut ? new Date(x.checkOut).toLocaleTimeString("en-IN") : "-"}</td>
                  <td>{x.workedMinutes || 0} min</td>
                  <td>
                  ...
                  <td>
                    <input
                      ...
                      onBlur={(e) => {
                        if (e.target.value !== (x.note || "")) mark(x, x.status, e.target.value);
                      }}
                    />
                  </td>
                  <td>
                    <select className="inline-select" value={x.status} onChange={(e) => mark(x, e.target.value)}>
        ...
        <Empty title="No attendance records" text="No employees have checked in for this date yet." />
      )}
    </>
  );
}
