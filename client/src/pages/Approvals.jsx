...
export default function Approvals(){
  ...
  return <>
    ...
    {!data.total?<Empty title="All clear" text="No pending approvals are waiting right now."/>:<div className="approval-grid">
      ...
      <section className="card approval-panel"><div className="section-head"><div><h3>Daily Work</h3><p>Submitted daily updates</p></div></div>{data.dailyWork.length?data.dailyWork.map(x=><article className="approval-item" key={x._id}><div><div className="row wrap"><h4>{x.employee?.name}</h4><Badge tone="purple">{x.progress}%</Badge></div><p>{x.workSummary||'No summary added'}</p><small>{x.date}</small></div><div className="approval-actions"><Button variant="primary" onClick={()=>act(()=>api.post('/daily-work/'+x._id+'/review',{decision:'approved',reviewNote:'Approved'}),'Daily work approved')}><CheckCircle2/>Approve</Button><Button className="btn orange" onClick={()=>act(()=>api.post('/daily-work/'+x._id+'/review',{decision:'rework',reviewNote:'Please update your daily work'}),'Daily rework sent')}><RotateCcw/>Rework</Button></div></article>):<Empty title="No daily updates" text="Submitted daily work will appear here."/>}</section>
    </div>}
  </>;
}
